...
fn main() {
    ...
    let space_name = "shifting2";
    let space = Space2D::by_file(format!("{}.2dspace",space_name).as_str(),15);
    let prob = ClusteringProblem{
        k : 5, // number of center;
        privacy_bound : 7, // number of points to represent;
        rep_interval : vec!((3,3),(0,0),(2,5)), // representation interval [a,b] for each color class; for color classes without interval we subsitute [0. inf]
    };
