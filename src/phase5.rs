...
use crate::types::{CenterIdx, ColorIdx, Distance, PointCount};
use crate::utilities;
...
pub(crate) fn phase5<M: ColoredMetric + std::marker::Sync>(
    space: &M,
    prob: &ClusteringProblem,
    centers_list: Vec<NewCenters>,
    clustering_list: Vec<Clustering>,
    spanning_trees: Vec<RootedSpanningTree>,
    thread_count: usize,
    phase_5_gonzalez: bool,
) -> (CenterIdx, Clustering, Distance) {
    ...
    thread_pool.scope(|pool| {
        ...
        for (i, new_centers) in centers_list.into_iter().enumerate() {
            ...
            pool.spawn(move |_| {
                ...
                let spread_centers = if phase_5_gonzalez {
                    spread_centers_within_cluster(space_ref, &clustering, &new_centers)
                } else {
                    new_centers
                };

                // now assign the points of each cluster the new centers and compute the radius:
                let (radius, new_clustering) = assign_points_to_new_centers(
                    space_ref,
                    prob_ref,
                    i,
                    clustering,
...
fn point_shifting<M: ColoredMetric>(
    space: &M,
...
fn hand_over<M: ColoredMetric>(
    ...
    let mut point_center_pairs: Vec<PointCenterLink> = clustering
        .get_cluster_of(supplier, space)
        .iter()
        .cloned()
        .map(|p| PointCenterLink {
            dist: space.dist(p, clustering.get_centers().get(consumer, space)),
            point: p,
            center_idx: consumer,
        })
        .collect();
    ...
    for PointCenterLink {
        ...
        point: p,
        center_idx: _,
    } in point_center_pairs
    {
        clustering.assign(p, consumer, space);
    }
}

fn spread_centers_within_cluster<M: ColoredMetric>(
    space: &M,
    pushed_clustering: &Clustering,
    phase4_centers: &NewCenters,
) -> NewCenters {
    ...
    let mut spread_centers_of_cluster =
        Vec::with_capacity(phase4_centers.new_centers_of_cluster.len());

    for j in 0..pushed_clustering.m() {
        // go through all clusters
        let clients = pushed_clustering.get_cluster_of(j, space);
        let old_centers: Vec<&Point> = phase4_centers
            .new_centers_of_cluster
            .get(j)
            .unwrap()
            .iter()
            .map(|&idx| phase4_centers.as_points.get(idx, space))
            .collect();
        ...
        let mut colors_to_be_opened: HashMap<ColorIdx, PointCount> =
            HashMap::with_capacity(old_centers.len());
        ...
        *colors_to_be_opened
            .get_mut(&space.color(first_center.unwrap()))
            .unwrap() -= 1;

        let mut dist_x_center: Vec<Distance> = vec![Distance::MAX; clients.len()];

        for r in 1..old_centers.len() {
            // one center opened already old_centers.len() - 1 to go
            ...
            for (s, client) in clients.iter().enumerate() {
                ...
                if dist_x_center[s] > candidate_dist
                    && colors_to_be_opened.contains_key(&color)
                    && *colors_to_be_opened.get(&color).unwrap() > 0
                {
            ...
            if candidate.is_none() {
                ...
                for old_center in old_centers.iter() {
                    let dist_to_chosen_centers =
                        space.dist_set(old_center, spread_centers.get_all(space));

                    let color = space.color(old_center);
                    if dist_to_chosen_centers > candidate_dist
                        && colors_to_be_opened.contains_key(&color)
                        && *colors_to_be_opened.get(&color).unwrap() > 0
                    {
            ...
            *colors_to_be_opened
                .get_mut(&space.color(candidate.unwrap()))
                .unwrap() -= 1;
        }
...
fn assign_points_to_new_centers<M: ColoredMetric>(
    space: &M,
    prob: &ClusteringProblem,
    i: CenterIdx,
    ...
    for cluster in 0..i + 1 {
        ...
        for point in points.iter() {
            ...
            link_to_nearest.push(PointCenterLink {
                point,
                center_idx: current_closest,
                dist: current_dist,
            });
        }
        ...
        let mut num_points_assigned_to: HashMap<CenterIdx, PointCount> =
            HashMap::with_capacity(center_idx.len());
        ...
        let batch_size = if prob.privacy_bound > safety_n_divided_k {
            prob.privacy_bound
        } else {
            safety_n_divided_k
        };
            ...
            for entry in batch {
                ...
                *counter += 1;
                if *counter == prob.privacy_bound {
                    ...
                    let idx = non_private_centers
                        .iter()
                        .position(|c| *c == chosen_center)
                        .unwrap();
    ...
    (
        radius,
        Clustering::new(centers.as_points, new_assignment, space),
    )
}
