...
impl<'a> State<'a> {
    ...
    fn clean_reassign(&mut self, c1: CenterIdx, c2: CenterIdx) {
        // removes invalid entries at the front
...
pub(super) fn initialize_state<'a>(
    ...
    State {
        ...
        next_to_non_private: (0..k)
            ...
            .collect(), // in the beginning all centers are non_private (except when the privacy bound is zero)
...
pub(super) fn add_edge<'a>(
    ...
    if c > i {
        // this edge is not considered yet
        return;
    }
...
pub(super) fn remove_edge<'a>(
    ...
    if c > i {
        // this edge has not been considered yet, and hence is not in the network
        return;
    }
