...
pub(super) fn split_in_half<'a, E: Clone + PartialOrd + Send>(list: &'a mut [E]) -> (&'a mut [E], &'a mut [E]) {
    split_in_two_at(list, (list.len() - 1) / 2 + 1)
}

pub(super) fn split_in_two_at<'a, E: Clone + PartialOrd + Send>(list: &'a mut [E], t: usize) -> (&'a mut [E], &'a mut [E]) {
    if 0 < t && t < list.len() {
        list.select_nth_unstable_by(t, |a, b| a.partial_cmp(b).unwrap());
